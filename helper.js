...
function NYLerpAngle (_angleA, _angleB, _t)
{
  let angleA = _angleA;
  let angleB = _angleB;

  let angleDiff = abs(angleB - angleA);

  if (abs((angleB - 360) - angleA) < angleDiff) {
    angleB -= 360;
  }
  else if (abs((angleB + 360) - angleA) < angleDiff) {
    angleB += 360;
  }
  else {
    return lerp(_angleA, _angleB, _t);
  }
